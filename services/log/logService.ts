import { injectable, multiInject, optional, type interfaces } from "inversify";
...
export function registerLogService(
  ...
  providers: interfaces.Newable<LogProvider>[]
) {
  ...
  container.registerDynamic(ScopedLogger, (ctx: interfaces.Context) => {
    ...
    if (loggerName === null) {
      throw new Error(
        "ScopedLogger must be injected with @named('LoggerName')"
      );
    }
