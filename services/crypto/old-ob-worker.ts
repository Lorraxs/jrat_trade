...
export class MarketStructure {
  ...
  async init(key: string) {
    ...
    for (const kline of klines) {
      convertedData.high.push(parseFloat(kline.high));
      convertedData.low.push(parseFloat(kline.low));
      convertedData.close.push(parseFloat(kline.close));
      convertedData.open.push(parseFloat(kline.open));
    }
...
self.onmessage = async (e: MessageEvent<IOldObWorkerMessage>) => {
  //console.log(e.data);
  switch (e.data.event) {
    case "start":
      const instance = new MarketStructure();
      const result = await instance.init(e.data.data);
      console.log(result);
      postMessage({
        event: "done",
        data: result,
